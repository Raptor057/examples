...
export function I18nProvider({ children }) {
  ...
  const value = useMemo(() => {
    ...
    const tCount = (key, count, vars) => {
      ...
      const suffixed = `${key}_${rule}`
      const template = dictionary[suffixed] ?? dictionary[`${key}_other`] ?? fallback[`${key}_other`] ?? key
    ...
    return {
      ...
      formatMoney: (value) => formatMoney(locale, value),
      formatDateTime: (value) => formatDateTime(locale, value),
      formatMonth: (monthNumber) => formatMonth(locale, monthNumber),
    }
  }, [locale])
...
// Numeros, fechas y moneda salen del locale activo, nunca de un formato fijo.
function formatNumber(locale, value) {
  if (value === null || value === undefined) return ''
  return new Intl.NumberFormat(locale).format(Number(value))
}

function formatMoney(locale, value) {
  if (value === null || value === undefined) return ''
  return new Intl.NumberFormat(locale, { style: 'currency', currency: 'MXN' }).format(Number(value))
}

// El backend serializa en UTC con sufijo Z; el front es la UNICA capa que convierte a la hora
// local de quien mira.
...
// El servidor manda el mes como NUMERO. El nombre lo pone el cliente en su idioma.
function formatMonth(locale, monthNumber) {
  const month = Number(monthNumber)
  if (!Number.isFinite(month) || month < 1 || month > 12) return String(monthNumber ?? '')
