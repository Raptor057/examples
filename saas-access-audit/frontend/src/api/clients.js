 ...
 * fetch por su cuenta. Aqui se adjunta el token y aqui -y solo aqui- se desenvuelve el envelope
 * { data, isSuccess, message, utcTimeStamp }.
 */
...
/**
 * Desenvuelve el envelope. Un fallo de negocio esperado llega como HTTP 200 con isSuccess:false,
 * asi que 200 NO significa exito: hay que leer isSuccess.
 *
 * El exito PARCIAL -la accion ocurrio pero su efecto externo quedo pendiente- llega como
 * isSuccess:true CON mensaje. Por eso esta funcion devuelve data y mensaje juntos: si devolviera
 * solo data, la pantalla no tendria de donde sacar la advertencia.
 */
...
export function extractApiErrorMessage(response, payload) {
  ...
  if (response?.status === 403) return 'No tienes permiso para esta operacion.'
